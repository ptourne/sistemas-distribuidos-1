...
const (
	...
	closeNotificationSendEOFToPeers
)
...
func Connector(serviceName string) (*RabbitMQConnector, error) {
	return ConnectorCustomWithName(serviceName, DefaultConfiguration())
}

func ConnectorCustom(config Configuration) (*RabbitMQConnector, error) {
	return ConnectorCustomWithName("", config)
}

func ConnectorCustomWithName(serviceName string, config Configuration) (*RabbitMQConnector, error) {
	url := Url(config)
...
func (m *middlewareRabbitmq[T]) WriteToRK(outputName string, subscribers map[string][]string, t string) (middleware.Sender[T], error) {
	...
	for sub, routingKeys := range subscribers {
		for _, routingKey := range routingKeys {
			q, ch, err := m.createQueueRK(outputName, sub, t, routingKey)
			if err != nil {
				return nil, fmt.Errorf("cannot create subscriber %s: %v", sub, err)
			}
			suscriberQueues = append(suscriberQueues, q.Name)
			ch.Close()
		}
	}
	...
	sender := &SenderRabbitmq[T]{
		...
		output:          output,
		Log:             m.Log,
		suscriberQueues: suscriberQueues,
	}
...
func (r *receiverRabbitmq[T]) handleFinishNotification(ok bool, msg amqp.Delivery) (
	...
	case closeNotificationFinishCidDone:
		...
		if !exists {
			...
			if r.finishCids[cid].finishDonePending == 0 {
				...
				if strings.Contains(name, "joiner") {
					r.closeSender.Publish(context.Background(), &CloseNotification{closeNotificationSendEOFToPeers}, cid)
					r.sentEOF = true
				}
	...
	case closeNotificationSendEOFToPeers:
		r.Log.Infof("Received Send EOF to peers for Cid %s | Receiver %s", cid, name)
		if !r.sentEOF {
			return false, true, newEOFEnvelope[T](cid), nil
		} else {
			r.sentEOF = false
		}
	}
