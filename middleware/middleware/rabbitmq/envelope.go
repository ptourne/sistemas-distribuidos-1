...
// =========
func newPruneEnvelope[T codec.Serializable[T]](
	cid string,
	ackFunc func() error,
) middleware.Envelope[T] {
	return &pruneEnvelopeRabbitmq[T]{
		t:       middleware.Prune,
		ackFunc: ackFunc,
	}
}

type pruneEnvelopeRabbitmq[T codec.Serializable[T]] struct {
	cid     string
	t       middleware.TypeMsg
	ackFunc func() error
}

func (r *pruneEnvelopeRabbitmq[T]) Msg() T {
	var nul T
	return nul
}

func (r *pruneEnvelopeRabbitmq[T]) Cid() string {
	return r.cid
}

func (r *pruneEnvelopeRabbitmq[T]) Type() middleware.TypeMsg {
	return r.t
}

func (r *pruneEnvelopeRabbitmq[T]) Ack(multiple bool) error {
	err := r.ackFunc()
	if err != nil {
...
func (r *pruneEnvelopeRabbitmq[T]) Nack(multiple bool) error {
	return fmt.Errorf("nack not implemented")
}
