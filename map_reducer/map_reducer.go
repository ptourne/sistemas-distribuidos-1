...
type MapReducer[I codec.Serializable[I], A codec.Serializable[A], R codec.Serializable[R]] struct {
	...
	connPartialResult         middleware.Connection[A]
}
...
func NewMapReducer[I codec.Serializable[I], A codec.Serializable[A], R codec.Serializable[R]](
	...
	inputCh, err := connIn.ConsumeFrom(input, name, routingKey, prefetchIn, shardCount)
	if err != nil {
	...
	partialResultIn, err := connPartialResult.ConsumeFrom(partialResultName, name, id, prefetch, shardCount)
	if err != nil {
		return nil, fmt.Errorf("failed to create accumulator input channel: %w", err)
	}
	partialResultOut, err := connPartialResult.WriteTo(partialResultName, []string{}, id, shardCount)
	if err != nil {
	...
	if id == "0" {
		for i := range shardCount {
			...
			finalReduceIn, err := connFinalReduce.ConsumeFrom(finalReuceName, name, rk, prefetch, shardCount)
			if err != nil {
				return nil, fmt.Errorf("failed to create accumulator input channel: %w", err)
			}
	...
	finalReduceOut, err := connFinalReduce.WriteTo(finalReuceName, []string{}, id, shardCountOutput)
	if err != nil {
		return nil, fmt.Errorf("failed to create accumulator output channel: %w", err)
	}
	...
	return &MapReducer[I, A, R]{
		...
		connPartialResult:         connPartialResult,
	}, nil
}
...
func (mr *MapReducer[I, A, R]) Run(ctx context.Context) error {
	...
	var err error
	for inputChan != nil || accBatch != nil || finalReduce != nil {
		select {
		case err = <-inputChan:
...
func (mr *MapReducer[I, A, R]) reduceBattchess(ctx context.Context) <-chan error {
	...
	task := func() {
		...
		for {
			...
			case <-ctx.Done():
				err = ctx.Err()
				mr.log.Infof("reduc : Received SIGTERM. Shutting down gracefully...")
				return
			default:
...
func (mr *MapReducer[I, A, R]) ReduceAndSend(cid string, msg A) error {
	...
	if ok {
		...
		mr.log.Infof("reduc : %s | ReduceAndSend | Final result sent | rk %s:", cid, mr.RoutingKey)
		return nil
	}
...
func (mr *MapReducer[I, A, R]) Close() {
	...
	if err := mr.PartialResultReceiver.Close(); err != nil {
		mr.log.Errorf("Error closing partial result channel (receiver): %s", err)
	}

	if err := mr.PartialResultSender.Close(); err != nil {
		mr.log.Errorf("Error closing partial result channel (sender): %s", err)
	}
	...
	if err := mr.connPartialResult.Close(); err != nil {
		mr.log.Errorf("Error closing partial result connection: %s", err)
	}
